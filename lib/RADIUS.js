 ...
 */
var freeradius_client = require("../build/Release/radius-ng"),
     sys = require('util');
/**
...
var ConnectionPool = function ConnectionPool(config){
    ...
    function request(type, attrs, cb){
	    ...
	    //if attributes in callback, mung them! Done here not in C++ because it is so much easier!
            if(args[1]){
		var tmp = {};
                for(var i=0,l=args[1].length;i<l;i++){
                    var name = args[1][i][0], value = args[1][i][1];
    ...
    self.genID = function genID() {
        return new Date().getTime() + process.pid + session_id_counter++;
    };
